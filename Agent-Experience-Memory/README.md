...
- Python 编排层：
  ...
  - 经验检索 Client
  - prompt context 构造
  - Python API Service，对外提供 HTTP 经验记忆接口
  - 端到端 demo
- C++ Memory Service：
  - 内存向量索引
  - cosine similarity
...
- Python API Service 负责对外暴露 HTTP 接口，外部 Agent 项目只通过 HTTP 调用经验记忆能力，不 import 本项目 Python 内部模块。
...
推荐使用 `start.sh` 一键管理 MySQL、C++ Memory Service 和 Python API Service：
...
`./start.sh start` 会按顺序执行：可选安装依赖、启动 MySQL、可选写入 seed 数据、构建 C++ 服务、启动 C++ 服务、加载 MySQL 向量到 C++ index、启动 Python API Service，并输出两个 health endpoint：
...
It uses `CPP_MEMORY_SERVICE_URL` to find the C++ Memory Service. If unset, it defaults to:
...
### 9. Run Python API search demo

```bash
NO_PROXY="127.0.0.1,localhost" no_proxy="127.0.0.1,localhost" python -m python_client.demo_api_search
```

The demo calls `http://127.0.0.1:8090/memory/search_context` and prints the returned context and memories.
...
Python API Service 将经验记忆能力封装为 HTTP API，默认监听 `8090`，外部 Agent 项目应通过 HTTP 调用本服务，而不是 import `python_client` 内部模块。

启动顺序可由 `./start.sh start --install-deps --seed` 自动完成，也可以手动执行：
...
3. 启动 C++ Memory Service `8080`。

4. 将 MySQL 中的经验向量加载到 C++ Memory Service：

```bash
python -m python_client.load_cpp_index
```

5. 启动 Python API Service `8090`：
...
```bash
uvicorn python_client.api_service:app --host 0.0.0.0 --port 8090
```

如果 C++ Memory Service 不在默认地址 `http://127.0.0.1:8080`，启动前设置：

```bash
export CPP_MEMORY_SERVICE_URL="http://127.0.0.1:8080"
```
